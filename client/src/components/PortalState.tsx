...
export function PortalError() {
  const { t, dir } = useLocale();
  return <div className="flex min-h-[55vh] items-center justify-center px-5" dir={dir}><section className="max-w-md rounded-3xl border border-amber-200 bg-white p-7 text-center shadow-sm dark:border-amber-800/50"><CircleAlert className="mx-auto h-8 w-8 text-amber-700" /><h1 className="mt-4 text-xl font-extrabold text-slate-900">{t.state.loadErrorTitle}</h1><p className="mt-2 text-sm leading-6 text-slate-600">{t.state.loadErrorBody}</p></section></div>;
}
