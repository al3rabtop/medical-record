    ...
    <div className="min-h-screen overflow-x-hidden bg-[#f7f9f7] text-slate-900" dir="rtl">
      <header className="sticky top-0 z-40 border-b border-white/80 bg-[#f7f9f7]/95 backdrop-blur-xl">
        ...
        <nav className="container flex gap-2 overflow-x-auto pb-3 lg:hidden" aria-label="بوابات السجل على الهاتف">
          ...
          <Link href="/settings" className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50" title="إعدادات الحساب"><Settings className="h-4 w-4" /></Link><Link href="/settings" className="flex shrink-0 items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600"><Settings className="h-3.5 w-3.5" />الإعدادات</Link>{isAdmin && <Link href="/admin" className="flex shrink-0 items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600"><Shield className="h-3.5 w-3.5" />الإحصائيات</Link>}<button onClick={handleLogout} className="flex shrink-0 items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600"><LogOut className="h-3.5 w-3.5" />خروج</button>
        </nav>
      </header>
      <main>{children}</main>
