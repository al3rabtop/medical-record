  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="max-w-32 truncate">{activeProfile?.name ?? t.profileSwitcher.chooseProfile}</span>
        <ChevronDown className="h-3.5 w-3.5 text-slate-400" />
      </button>

      {open && (
        <div className="absolute start-0 z-50 mt-2 w-64 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-lg">
          <p className="border-b border-slate-100 px-4 py-2.5 text-[11px] font-bold text-slate-500">
            {t.profileSwitcher.healthProfiles}
          </p>
          {profiles.map(p => (
            <button
              key={p.id}
            ...
            >
              <span className="min-w-0">
                ...
                <span className="block text-[11px] text-slate-500">
                  {p.relation ?? "—"}
                  {p.birthYear ? ` · ${p.birthYear}` : ""} · {t.profileSwitcher.reportSuffix(p.visitCount)}
                </span>
              </span>
